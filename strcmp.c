...
double EMSCRIPTEN_KEEPALIVE strcmp95(char *ying, char *yang, long y_length, int ind_c[])
// double strcmp95(char *ying, char *yang, long y_length, int ind_c[])
{
  // printf("%i - %i,%i,%i\n", ind_c, ind_c[0], ind_c[1], ind_c[2]);
/* Arguments:
...
/* Optionally adjust for similarities in nonmatched characters                */
if (!ind_c[2]) {
  N_simi = 0;
  if (minv > Num_com) {
    for (i = 0;i < ying_length;i++) {
...
//   if (argv[3] != NULL && strcmp(argv[3], "--no-length") == 0)
//   {
//     options[0] = 1;
//   }

//   if (argv[4] != NULL && strcmp(argv[4], "--simil") == 0)
//   {
//     options[2] = 0;
//   }

//   printf("%f", strcmp95(argv[1], argv[2], strlen(argv[2]), options));
